...
class CsvAnalysisAgent:
    ...
    def _make_graph(
        ...
        def steps_to_suggest(state: CsvState) -> dict[str, Any]:
            ...
            return steps_to_suggest_tool.invoke(
                {"motif": state["motif"], "report": state.get("report")}
            )
        ...
        graph.add_conditional_edges(
            ...
            {
                "execute": "execute",
                "steps_to_suggest": "steps_to_suggest",
                "failed": END,
            },
        )
        graph.add_conditional_edges(
        ...
        graph.add_conditional_edges(
            "summary",
            after_summary,
            {"steps_to_suggest": "steps_to_suggest", "failed": END},
        )
    ...
    def run(
        ...
        state = graph.invoke(
            {"motif": motif, "objective": objective, "profile": profile}
        )
